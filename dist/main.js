...
async function getRates() {
    if (!_rates) {
        _rates = await fetch('https://cdn.jsdelivr.net/gh/fawazahmed0/currency-api@1/latest/currencies/usd.min.json')
...
async function processElement(element) {
    ...
    for (const child of Array.from(element.childNodes)) {
        if (child instanceof Text && child.data.trim()) {
            ...
            const match = originalPriceStr.match(/^(?<currency>[^0-9,.]+)(?<amount>[0-9,.]+)(?<reduced> \(Reduced .+\))?$/);
            if (!match) {
                continue;
            }
...
async function processListings() {
    ...
    await Promise.all(Array.from(articles).map(async (article) => {
...
async function processBySelector(selector) {
    const elements = document.querySelectorAll(selector);
    await Promise.all(Array.from(elements).map((element) => processElement(element)));
}
